...
package one.block.arisenjava.utilities;
...
import one.block.arisenjava.error.utilities.*;
 ...
 * from formats that are germane to rix blockchain transactions and the PEM (Privacy Enhanced Mail)
 * format for those objects.  It also provides methods to format a serialized transaction into a
 * format that can be submitted to an rix blockchain.
 */
public class RIXFormatter {

    /*
    rix Format Prefixes - The prefixes below are all used to preface the rix format of certain types
    of keys and signatures.  For instance, 'rix' is used to preface a legacy form of a public key
    that was generated using the secp256k1 algorithm.  The prefixes and there associated objects are
    as follows:
    rix - Public Key generated with secp256k1 algorithm formatted for use on rix blockchain.
    PUB_R1_ - Public Key generated with secp256r1 or prime256v1 algorithm formatted for use on rix blockchain.
    PUB_K1_ - Public Key generated with secp256k1 algorithm formatted for use on rix blockchain.
    PVT_R1_ - Private Key generated with secp256r1 algorithm formatted for use on rix blockchain.
     ...
     */
    private static final String PATTERN_STRING_RIX_PREFIX_RIX = "RIX";
    private static final String PATTERN_STRING_RIX_PREFIX_PUB_R1 = "PUB_R1_";
    private static final String PATTERN_STRING_RIX_PREFIX_PUB_K1 = "PUB_K1_";
    private static final String PATTERN_STRING_RIX_PREFIX_PVT_R1 = "PVT_R1_";
    private static final String PATTERN_STRING_RIX_PREFIX_SIG_R1 = "SIG_R1_";
    private static final String PATTERN_STRING_RIX_PREFIX_SIG_K1 = "SIG_K1_";
    ...
    //CONSTANTS USED DURING rix ENCODING
    private static final int RIX_SECP256K1_HEADER_BYTE = 0x80;

    //CONSTANTS USED DURING rix DECODING
    ...
    /**
     * This method converts a PEM formatted public key to the RIX format.
     *
     ...
     * secp256k1 key that uses a "RIX" prefix.
     * @return RIX formatted public key as string
     * @throws RIXFormatterError if PEM conversion to RIX format fails.
     */
    @NotNull
    public static String convertPEMFormattedPublicKeyToRIXFormat(@NotNull String publicKeyPEM,
            boolean requireLegacyFormOfSecp256k1Key)
            throws RIXFormatterError {
        String rixFormattedPublicKey = publicKeyPEM;
         ...
         */
        String type;
        try (Reader reader = new CharArrayReader(rixFormattedPublicKey.toCharArray());
                PemReader pemReader = new PemReader(reader);) {
            pemObject = pemReader.readPemObject();
            type = pemObject.getType();
        } catch (Exception e) {
            throw new RIXFormatterError(ErrorConstants.INVALID_PEM_PRIVATE_KEY, e);
        }
        ...
        if (type.matches("(?i:.*" + PEM_HEADER_EC_PUBLIC_KEY + ".*)")) {
            ...
            if (rixFormattedPublicKey
                ...
                throw new RIXFormatterError(ErrorConstants.INVALID_DER_PRIVATE_KEY);
            }
            ...
            */
            byte[] rixFormattedPublicKeyBytes = Hex.decode(rixFormattedPublicKey);
            if (rixFormattedPublicKeyBytes[0] == UNCOMPRESSED_PUBLIC_KEY_BYTE_INDICATOR) {
                try {
                    rixFormattedPublicKey = Hex.toHexString(
                            compressPublickey(Hex.decode(rixFormattedPublicKey),
                                    algorithmEmployed));
                } catch (Exception e) {
            ...
            try {
                ...
                rixFormattedPublicKey = encodePublicKey(Hex.decode(rixFormattedPublicKey),
            ...
            throw new RIXFormatterError(ErrorConstants.INVALID_PEM_PRIVATE_KEY);
        }

        return rixFormattedPublicKey;
    }

    /**
     * This method converts an rix formatted public key to the PEM format.
     *
     * @param publicKeyRIX Public key in the rix format
     * @return PEM formatted public key as string
     * @throws RIXFormatterError if rix conversion to PEM format fails.
     */
    @NotNull
    public static String convertRIXPublicKeyToPEMFormat(@NotNull String publicKeyRIX)
            throws RIXFormatterError {
        String pemFormattedPublickKey = publicKeyRIX;
        AlgorithmEmployed algorithmEmployed;
        String keyPrefix;

        /*
        The public key will contain an rix prefix indicating which algorithm was used to generate it.
        Below we split the prefix from the key.
         */
        if (pemFormattedPublickKey.toUpperCase()
                .contains(PATTERN_STRING_RIX_PREFIX_PUB_R1.toUpperCase())) {
            algorithmEmployed = AlgorithmEmployed.SECP256R1;
            keyPrefix = PATTERN_STRING_RIX_PREFIX_PUB_R1;
            pemFormattedPublickKey = pemFormattedPublickKey
                    .replace(PATTERN_STRING_RIX_PREFIX_PUB_R1, "");
        } else if (pemFormattedPublickKey.toUpperCase()
                .contains(PATTERN_STRING_RIX_PREFIX_PUB_K1.toUpperCase())) {
            algorithmEmployed = AlgorithmEmployed.SECP256K1;
            keyPrefix = PATTERN_STRING_RIX_PREFIX_PUB_K1;
            pemFormattedPublickKey = pemFormattedPublickKey
                    .replace(PATTERN_STRING_RIX_PREFIX_PUB_K1, "");
        } else if (pemFormattedPublickKey.toUpperCase()
                .contains(PATTERN_STRING_RIX_PREFIX_RIX.toUpperCase())) {
            algorithmEmployed = AlgorithmEmployed.SECP256K1;
            keyPrefix = PATTERN_STRING_RIX_PREFIX_RIX;
            pemFormattedPublickKey = pemFormattedPublickKey
                    .replace(PATTERN_STRING_RIX_PREFIX_RIX, "");
        } else {
            throw new RIXFormatterError(ErrorConstants.INVALID_RIX_PUBLIC_KEY);
        }
        ...
        try {
            ...
            throw new RIXFormatterError(ErrorConstants.BASE58_DECODING_ERROR, e);
        }
        ...
        switch (algorithmEmployed) {
            ...
            case SECP256K1:
                ...
                break;
            default:
                throw new RIXFormatterError(ErrorConstants.UNSUPPORTED_ALGORITHM);
        }
    ...
    /**
     * This method converts a signature to a rix compliant form.  The signature to be converted must
     ...
     * @return RIX format of signature
     * @throws RIXFormatterError if DER conversion to RIX format fails.
     */
    @NotNull
    public static String convertDERSignatureToRIXFormat(@NotNull byte[] signatureDER,
            @NotNull byte[] signableTransaction, @NotNull String publicKeyPEM)
            throws RIXFormatterError {
        String rixFormattedSignature = "";
        ...
        try (ASN1InputStream asn1InputStream = new ASN1InputStream(signatureDER)) {
            ...
            switch (algorithmEmployed) {
                case SECP256R1:
                    ...
                    signaturePrefix = PATTERN_STRING_RIX_PREFIX_SIG_R1;
                    break;
                case SECP256K1:
                    signatureWithCheckSum = addCheckSumToSignature(decodedSignature,
                            SECP256K1_CHECKSUM_VALIDATION_SUFFIX.getBytes());
                    signaturePrefix = PATTERN_STRING_RIX_PREFIX_SIG_K1;
                    break;
                default:
                    throw new RIXFormatterError(ErrorConstants.UNSUPPORTED_ALGORITHM);
            ...
            //Base58 encode signature and add pertinent rix prefix
            rixFormattedSignature = signaturePrefix.concat(Base58.encode(signatureWithCheckSum));

        } catch (Exception e) {
            throw new RIXFormatterError(ErrorConstants.SIGNATURE_FORMATTING_ERROR, e);
        }

        return rixFormattedSignature;
    }
     ...
     * @return rix format of signature
     * @throws RIXFormatterError if conversion to rix format fails.
     */
    @NotNull
    public static String convertRawRandSofSignatureToRIXFormat(@NotNull String signatureR,
            String signatureS,
            @NotNull byte[] signableTransaction, @NotNull String publicKeyPEM)
            throws RIXFormatterError {
        String rixFormattedSignature = "";
        ...
        try {
            ...
            if (algorithmEmployed.equals(AlgorithmEmployed.SECP256K1) &&
                    !isCanonical(decodedSignature)) {
                throw new RixFormatterSignatureIsNotCanonicalError(ErrorConstants.NON_CANONICAL_SIGNATURE);
            }
    ...
    /**
     * This method converts a PEM formatted private key to the rix format.
     *
     * @param privateKeyPEM Private key in PEM format
     * @return rix formatted private key as string
     * @throws RIXFormatterError if PEM conversion to rix format fails.
     */
    @NotNull
    public static String convertPEMFormattedPrivateKeyTorixFormat(@NotNull String privateKeyPEM)
            throws RIXFormatterError {
        String rixFormattedPrivateKey = privateKeyPEM;
        AlgorithmEmployed algorithmEmployed;
        PemObject pemObject;
         ...
         */
        String type;
        try (Reader reader = new CharArrayReader(rixFormattedPrivateKey.toCharArray());
                PemReader pemReader = new PemReader(reader);) {
            pemObject = pemReader.readPemObject();
            type = pemObject.getType();
        } catch (Exception e) {
        ...
        if (type.matches("(?i:.*" + PEM_HEADER_EC_PRIVATE_KEY + ".*)")) {
            ...
            rixFormattedPrivateKey = Hex.toHexString(pemObject.getContent());

            //Determine algorithm used to generate key
            if (rixFormattedPrivateKey
                    .matches("(?i:.*" + PATTERN_STRING_PEM_SUFFIX_PRIVATE_KEY_SECP256R1 + ".*)")) {
                algorithmEmployed = AlgorithmEmployed.SECP256R1;
            } else if (rixFormattedPrivateKey
            ...
            switch (algorithmEmployed) {
                case SECP256R1:
                    rixFormattedPrivateKey = rixFormattedPrivateKey
                            .substring(PATTERN_STRING_PEM_PREFIX_PRIVATE_KEY_SECP256R1.length(),
                                    rixFormattedPrivateKey.length()
            ...
            try {
                ...
                rixFormattedPrivateKey = encodePrivateKey(Hex.decode(rixFormattedPrivateKey),
                        algorithmEmployed);
            } catch (Base58ManipulationError e) {
                throw new RIXFormatterError(e);
            }
            ...
            //Add prefix
            StringBuilder builder = new StringBuilder(rixFormattedPrivateKey);
            switch (algorithmEmployed) {
                case SECP256K1:
                    //K1 keys do not currently use prefixes
                    break;
                case SECP256R1:
                    builder.insert(0, PATTERN_STRING_RIX_PREFIX_PVT_R1);
                    break;
                default:
                    break;
            }
            rixFormattedPrivateKey = builder.toString();

        } else {
            throw new RIXFormatterError(ErrorConstants.INVALID_PEM_PRIVATE_KEY);
        }

        return rixFormattedPrivateKey;
    }

    /**
     * This method converts an rix formatted private key to the PEM format.
     *
     * @param privateKeyRIX Private key in rix format
     * @return PEM formatted private key as a string
     * @throws RIXFormatterError if rix conversion to PEM format fails.
     */
    @NotNull
    public static String convertRIXPrivateKeyToPEMFormat(@NotNull String privateKeyRIX)
            throws RIXFormatterError {
        String pemFormattedPrivateKey = privateKeyRIX;
        AlgorithmEmployed algorithmEmployed;
        ...
        if (pemFormattedPrivateKey.toUpperCase()
                .contains(PATTERN_STRING_RIX_PREFIX_PVT_R1.toUpperCase())) {
            algorithmEmployed = AlgorithmEmployed.SECP256R1;
            /*
            ...
            pemFormattedPrivateKey = pemFormattedPrivateKey
                    .split(PATTERN_STRING_RIX_PREFIX_PVT_R1)[1];
        } else {
            algorithmEmployed = AlgorithmEmployed.SECP256K1;
        }
        ...
        if (pemFormattedPrivateKey.length() > FIRST_TWO_BYTES_OF_KEY) {
            ...
            throw new RIXFormatterError(ErrorConstants.INVALID_RIX_PRIVATE_KEY);
        }
     ...
     *
     * @param rixTransaction - the input signable transaction
     * @return - extracted serialized transaction from the input signable transaction
     * @throws RIXFormatterError if input is invalid
     */
    public static String extractSerializedTransactionFromSignable(@NotNull String rixTransaction)
            throws RIXFormatterError {
        if (rixTransaction.isEmpty()) {
            throw new RIXFormatterError(ErrorConstants.EMPTY_INPUT_EXTRACT_SERIALIZIED_TRANS_FROM_SIGNABLE);
        }

        if (rixTransaction.length() <= MINIMUM_SIGNABLE_TRANSACTION_LENGTH) {
            throw new RIXFormatterError(String.format(ErrorConstants.INVALID_INPUT_SIGNABLE_TRANS_LENGTH_EXTRACT_SERIALIZIED_TRANS_FROM_SIGNABLE, MINIMUM_SIGNABLE_TRANSACTION_LENGTH));
        }

        if (!rixTransaction.endsWith(Hex.toHexString(new byte[32]))) {
            throw new RIXFormatterError(ErrorConstants.INVALID_INPUT_SIGNABLE_TRANS_EXTRACT_SERIALIZIED_TRANS_FROM_SIGNABLE);
        }

        try {
            String cutChainId = rixTransaction.substring(CHAIN_ID_LENGTH);
            return cutChainId.substring(0, cutChainId.length() - Hex.toHexString(new byte[32]).length());
        } catch (Exception ex) {
            throw new RIXFormatterError(ErrorConstants.EXTRACT_SERIALIZIED_TRANS_FROM_SIGNABLE_ERROR, ex);
        }
    }
     ...
     * @throws RIXFormatterError if inputs are invalid
     */
    public static String prepareSerializedTransactionForSigning(@NotNull String serializedTransaction,
            @NotNull String chainId) throws RIXFormatterError {
        if (serializedTransaction.isEmpty() || chainId.isEmpty()) {
            throw new RIXFormatterError(ErrorConstants.EMPTY_INPUT_PREPARE_SERIALIZIED_TRANS_FOR_SIGNING);
        }
    ...
    public static String encodePrivateKey(@NotNull byte[] pemKey,
        ...
        switch (keyType) {
            ...
            case PRIME256V1:
                ...
                break;
            case SECP256K1:
                pemKey = Bytes.concat(new byte[]{((Integer) RIX_SECP256K1_HEADER_BYTE).byteValue()},
                        pemKey);
    ...
    /**
     * Encoding PEM public key to RIX format.
     *
     ...
     * "RIX" prefix.
     * @return - RIX format of public key
    ...
    public static String encodePublicKey(@NotNull byte[] pemKey, @NotNull AlgorithmEmployed keyType,
        ...
        switch (keyType) {
            case SECP256K1:
                if (isLegacy) {
                    builder.insert(0, PATTERN_STRING_RIX_PREFIX_RIX);
                } else {
                    builder.insert(0, PATTERN_STRING_RIX_PREFIX_PUB_K1);
                }
                break;
            case SECP256R1:
                builder.insert(0, PATTERN_STRING_RIX_PREFIX_PUB_R1);
                break;
            default:
                break;
        }
     ...
     * @param keyPrefix RIX specific key type prefix (i.e. PUB_R1_, PUB_K1_, or RIX).
    ...
    public static byte[] decodePublicKey(@NotNull String strKey, String keyPrefix)
        ...
        try {
            ...
            switch (keyPrefix) {
                case PATTERN_STRING_RIX_PREFIX_PUB_R1:
                ...
                case PATTERN_STRING_RIX_PREFIX_PUB_K1:
                    if (invalidRipeMD160CheckSum(decodedKey, firstCheckSum,
                            SECP256K1_CHECKSUM_VALIDATION_SUFFIX.getBytes())) {
                        throw new IllegalArgumentException(
                                ErrorConstants.BASE58_INVALID_CHECKSUM);
                ...
                case PATTERN_STRING_RIX_PREFIX_RIX:
                    if (invalidRipeMD160CheckSum(decodedKey, firstCheckSum,
                            LEGACY_CHECKSUM_VALIDATION_SUFFIX.getBytes())) {
                        throw new IllegalArgumentException(
                                ErrorConstants.BASE58_INVALID_CHECKSUM);
     ...
     * @throws RIXFormatterError when public key decompression fails.
     */
    @NotNull
    private static byte[] decompressPublickey(byte[] compressedPublicKey,
        ...
        try {
            ...
            throw new RIXFormatterError(ErrorConstants.PUBLIC_KEY_DECOMPRESSION_ERROR, e);
        }
    }
     ...
     * @throws RIXFormatterError when public key compression fails.
     */
    @NotNull
    private static byte[] compressPublickey(byte[] compressedPublicKey,
        ...
        try {
            ...
            throw new RIXFormatterError(ErrorConstants.PUBLIC_KEY_COMPRESSION_ERROR, e);
        }
    }
